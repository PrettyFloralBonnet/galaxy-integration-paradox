...
class ParadoxClient:
    ...
    async def get_skus(self):
        if not self._skus:
            skus = await self.http_client.do_request(
                "GET",
                "https://accounts.paradoxplaza.com/api/skus",
    ...
    async def get_account_id(self):
        data = {
            "Authorization": f'{{"session":{{"token":"{self.http_client.token}"}}}}',
            "content-type": "application/json",
        }
        response = await self.http_client.do_request(
            "GET", "https://api.paradox-interactive.com/accounts", headers=data
        )
    ...
    async def get_owned_games(self):
        ...
        response = await self.http_client.do_request(
            "GET",
            "https://api.paradox-interactive.com/inventory/products",
            headers=data,
        )
                ...
                for platform in platforms:
                    for game in platforms[platform]:
                        ...
                        if game["sku"] and game["title"] and game["product_type"]:
                            if (
                                game["sku"] in skus
                                and "paradoxLauncher" in skus[game["sku"]]["platform"]
